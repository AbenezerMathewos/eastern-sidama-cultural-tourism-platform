...
exports.processHostMediaUpload = catchAsync(async (req, res, next) => {
  ...
  if (req.files) {
    ...
    if (req.files.hostingEnvironmentPhotos) {
      media.hostingEnvironmentPhotos = req.files.hostingEnvironmentPhotos.map(file => toStoredFileUrl(file));
    }
  }
...
exports.approveApplication = catchAsync(async (req, res, next) => {
  ...
  notifyHostApplicationApproved(user).catch(err => {
    if (process.env.NODE_ENV !== 'production') {
      console.error('Error creating host approval notification:', err);
    }
  });

  if (req.body.guideId) {
    User.findById(req.body.guideId).then(guide => {
      if (guide) return notifyGuideAssigned(user, guide);
    }).catch(err => {
      if (process.env.NODE_ENV !== 'production') {
...
exports.rejectApplication = catchAsync(async (req, res, next) => {
  ...
  notifyHostApplicationRejected(user, application.rejectionReason).catch(err => {
    if (process.env.NODE_ENV !== 'production') {
      console.error('Error creating host rejection notification:', err);
    }
  });
});
