...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    try {
      ...
      console.error('Email failed but user still created:', err.message);
    }
  } else {
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (roles.includes('host')) {
      ...
      return next(
        new AppError(
          'You do not have permission to perform this action. Approved host or admin required.',
          403
        )
      );
    }
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    try {
      ...
      // If welcome email fails, don't block verification
